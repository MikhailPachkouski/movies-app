...
const Series = () => {
	...
	const fetchSeries = async () => {
		const { data } = await axios.get(
			`https://api.themoviedb.org/3/discover/tv?api_key=${process.env.REACT_APP_API_KEY}&language=${locale}&sort_by=popularity.desc&include_adult=false&include_video=false&page=${page}&with_genres=${genreUrl}`
		);
	...
	return (
		<div>
			<div className='trending__title'>
				{locale === 'ru-RU' ? 'Сериалы по жанрам' : 'Series by genre'}
			</div>
			<div className='locale__wrapper'>
				<LocaleToggler />
			</div>
